<template>
    ...
    <div class="contact">
      <section class="contact-container">
        <form class="contact-form" v-on:submit.prevent="submit">
          ...
          <textarea cols="30" rows="5" class="textarea contact-input" v-model="textArea" v-on:blur="validTextArea" placeholder="Escríbenos para ayudarnos a mejorar"></textarea>
          <p v-if="textAreaError" class="error-message">Escribe al menos 3 palabras</p>
          <input type="email" class="contact-email contact-input" v-model="email" v-on:blur="validEmail" placeholder="Correo electrónico">
          <p v-if="emailError" class="error-message">El email no es valido.</p>
          <button type="submit" class="contact-submit">Enviar</button>
        </form>
      </section>
      <section class="contact-modal" v-if="submitted">
        <div class="contact-modal-container">
          <p class="contact-modal-message">Sugerencia enviada con exito</p>
          <RouterLink to="/" class="contact-redirect">Volver</RouterLink>
        </div>
      </section>
    </div>
  ...
  export default {
    ...
    //Estos watcher meten en las variables de datas el valor de los input cada vez que estos cambian
    watch:{
      textArea: function() {            
      },
      email: function() {           
